...
def generate():
    ...
    facebook_username = "9674838357"
    facebook_password = "sougata9674838357"
                        ...
                        for result in results:
                            ...
                            if args.mode == "fast":
                                if result < threshold:
                                    ...
                                    person.facebookimage = encoding.smart_str(image_link, encoding='ascii',
                                                                              errors='ignore')
                        ...
                        for result in results:
                            if args.mode == "fast":
                                if result < threshold:
                                    ...
                                    person.twitterimage = encoding.smart_str(image_link, encoding='ascii',
                                                                             errors='ignore')
                                    if args.vv == True:
                                        print("\tMatch found: " + person.full_name)
                ...
                if image_link:
                    try:
                        ...
                        for result in results:
                            if args.mode == "fast":
                                if result < threshold:
                                    person.instagram = encoding.smart_str(profilelink, encoding='ascii',
                                                                          errors='ignore')
                        ...
                        for result in results:
                            if args.mode == "fast":
                                if result < threshold:
                                    ...
                                    person.linkedinimage = encoding.smart_str(image_link, encoding='ascii',
                                                                              errors='ignore')
                                    if args.vv == True:
                                        print("\tMatch found: " + person.full_name)
                        ...
                        for result in results:
                            if args.mode == "fast":
                                if result < threshold:
                                    ...
                                    person.weiboimage = encoding.smart_str(image_link, encoding='ascii',
                                                                           errors='ignore')
                                    if args.vv == True:
                                        print("\tMatch found: " + person.full_name)
                        ...
                        for result in results:
                            if args.mode == "fast":
                                if result < threshold:
                                    ...
                                    person.doubanimage = encoding.smart_str(image_link, encoding='ascii',
                                                                            errors='ignore')
                                    if args.vv == True:
                                        print("\tMatch found: " + person.full_name)
    ...
    if not (args.a or args.fb or args.tw or args.ig or args.li or args.vk or args.wb or args.db):
        parser.error(
            'No sites specified requested, add -a for all, or a combination of the sites you want to check using a mix of -fb -tw -ig -li -vk -db -wb')
    ...
    peoplelist = []
    args.format = "imagefolder"
    args.input = "/home/sougata/Desktop/social_mapper/Input-Examples/imagefolder"
    ...
    if args.format == "company":
        ...
        if args.companyid is not None:  # Don't find company id, use provided id from -cid or --companyid flag
            ...
            for i in range(0, len(content['elements'])):
                try:
                    companyid = content['elements'][i]['hitInfo']['com.linkedin.voyager.typeahead.TypeaheadCompany'][
                        'id']
                    if firstID == 0:
                        firstID = companyid
            ...
            for c in content['elements'][0]['elements']:
                if 'com.linkedin.voyager.search.SearchProfile' in c['hitInfo'] and \
                    ...
                    try:
                        ...
                        first_name = c['hitInfo']['com.linkedin.voyager.search.SearchProfile']['miniProfile'][
                            'firstName']
                        first_name = encoding.smart_str(first_name, encoding='ascii', errors='ignore')
    ...
    if args.format == "socialmapper":
        ...
        for personhtml in tbodylist:
            ...
            person.facebookimage = encoding.smart_str(personhtml.findAll("td")[3].find("img")['src'], encoding='ascii',
                                                      errors='ignore').replace(";", "")
            person.facebookcdnimage = encoding.smart_str(personhtml.findAll("td")[3].find("img")['src'],
                                                         encoding='ascii', errors='ignore').replace(";", "")
    ...
    for person in peoplelist:
        body = "<tbody>" \
               ...
               "</tr>" \
               "</tbody>" % (person.person_imagelink, person.person_imagelink, person.full_name, person.linkedin,
                             person.linkedinimage, person.linkedin, person.facebook, person.facebookcdnimage,
                             person.facebook, person.twitter, person.twitterimage, person.twitter, person.instagram,
                             person.instagramimage, person.instagram, "", "", "", person.vk, person.vkimage, person.vk,
                             person.weibo, person.weiboimage, person.weibo, person.douban, person.doubanimage,
                             person.douban)
